...
def main(scheme):
    ...
    # aaaargh
    discretize(teval, scheme.create_collection("teval_discrete"),
               ['Media', 'Deviazione standard', 'N', 'P<6', 'P>=6'],
               [VAL_SCORE, STD_DEV, ZERO_TO_HUND, ZERO_TO_HUND, ZERO_TO_HUND],
               True)

    discretize(sprod, scheme.create_collection("sprod_discrete"),
               ['Media', 'Deviazione standard', 'N', 'P<24', 'P>=24'],
               [MARKS, STD_DEV, ZERO_TO_HUND, ZERO_TO_HUND, ZERO_TO_HUND],
               True)

    merge_teach(scheme.teval_discrete)
    the_big_merge(scheme.teval_discrete, scheme.sprod_discrete,
